...
$(document).ready(function () {
  ...
  function updateBackgroundColor() {
    var currentTime = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Riyadh" }));
    var hours = currentTime.getHours();

    // Set background color based on time of day
    if (hours >= 6 && hours < 18) {
      $('#clock5').css('background-color', '#fff'); // Daylight color
      $('#clock5').css('--primary-color', '#242526');
      $('#clock5').css('--white-color', '#18191a');
      $('#clock5').css('--black-color', '#fff');
      $('#clock5').css('--red-color', '#e74c3c');
    } else {
      $('#clock5').css('background-color', '#000'); // Nighttime color
      $('#clock5').css('--primary-color', '#242526');
      $('#clock5').css('--white-color', '#18191a');
      $('#clock5').css('--black-color', '#fff');
      $('#clock5').css('--red-color', '#e74c3c');
