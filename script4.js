$(document).ready(function () {
  ...
  function updateBackgroundColor() {
    var currentTime = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Detroit" }));
    var hours = currentTime.getHours();

    // Set background color based on time of day
    if (hours >= 6 && hours < 18) {
      $('#clock4').css('background-color', '#fff'); // Daylight color
      $('#clock4').css('--primary-color', '#242526');
      $('#clock4').css('--white-color', '#18191a');
      $('#clock4').css('--black-color', '#fff');
      $('#clock4').css('--red-color', '#e74c3c');
    } else {
      $('#clock4').css('background-color', '#000'); // Nighttime color
      $('#clock4').css('--primary-color', '#242526');
      $('#clock4').css('--white-color', '#18191a');
      $('#clock4').css('--black-color', '#fff');
      $('#clock4').css('--red-color', '#e74c3c');
